...
const PasswordlessSettingsPage = () => {
  const { formatMessage } = useIntl();
  const [showModalWarning, setShowModalWarning] = useState(false);
  const pageTitle = formatMessage({ id: getTrad('Header.Settings') });
  const formTitle = formatMessage({
    id: getTrad('Form.title.Settings'),
    defaultMessage: 'Settings',
  });
  const updatePermissions = useMemo(() => {
    return { update: pluginPermissions.updateSettings };
  }, []);
  const {
  ...
  const handleSubmit = useCallback(
    async e => {
      ...
      try {
        ...
        await request(getRequestURL('settings'), { method: 'PUT', body: modifiedData });
    ...
    <>
      <div className={"p-4"}>
        ...
        <PopUpWarning
          ...
          content={{
            title: getTrad('Warning.title'),
            message: getTrad('Warning.warning.cancel'),
            cancel: getTrad('Warning.button.cancel'),
            confirm: getTrad('Warning.button.confirm'),
          }}
...
export default PasswordlessSettingsPage;
